...
import pypulous.IterFromPoint
import pypulous.IterGrid
import pypulous.World
...
class WorldTests(unittest.TestCase):
	...
	def testNew(self):
		world = pypulous.World.World(width=self.world_width, height=self.world_height)
...
class IterFromPosTest(unittest.TestCase):
	...
	def doLoop(self, source, limit=None):
		count = 0
		for p in pypulous.IterFromPoint.IterFromPoint(source, self.world, limit):
			self.assertTrue(p)
			# print p.x, p.y
			count += 1
		return count
...
class IterGridTests(unittest.TestCase):
	...
	def testNew(self):
		my_iter = pypulous.IterGrid.IterGrid(self.world)
		count = 0
